...
public final class lineutility
{
    ...
    /**
     * Calculates the center point of an area using the first vblCounter
     * points in the array.
     *
    ...
    */
    ...
    /**
     * Calculates the slopes and y intercepts in pixels for the line
     * from pt1 to pt2 and a parallel line a vertical distance from the line
     *
    ...
    /**
     * Computes the arc points for FEBA
     * Assumes pResultLinePoints has been allocated.
     *
     * @param dRadius the arc radius in pixels
     * @param pLinePoints the client points, each one is a center for a computed arc (cricle)
    ...
    /**
     * Computes the "X" points for FEBA
     * Assumes pResultLinePoints has been allocated.
     *
     * @param pLinePoints the client points
     ...
     * @deprecated 
     * Returns a point extended perpendicularly from a line at a given direction
     * same as original function except it accounts for vertical lines and negative d values
     *
     * @param pt1 first line point
     * @param pt2 last line point
     * @param pt0 on line from which to extend
...
//                    nQuadrant=4;
    ...
    /**
     * Returns center point for a clockwise arc to connect pts 1 and 2.
     * Also returns an extended point on the line between pt1 and the new center
     * Caller passes a POINT1 array of size 2 for ptsSeize, passes pt1 and pt2 in ptsSeize
     * Returns the radius of the 90 degree arc between C (arc center) and pt1
     * 
     * @param ptsSeize OUT - two point array also used for the returned two points
     *
     * @return the radius
     */
                                            ...
                                            int styl)
    {
        try
        {
            ...
            tempLinePoint.x=(int)AHBLX;tempLinePoint.y=(int)AHBLY;
            pResultLinePoints[0]=new POINT2(tempLinePoint);
            pResultLinePoints[1]=new POINT2(endLinePoint);
            tempLinePoint.x=(int)AHBRX;tempLinePoint.y=(int)AHBRY;
    ...
    /**
     * Returns a point a distance d pixels perpendicular
     * to the pt0-pt1 line and going toward pt2
     *
     * @param pt0 the first line point
     * @param pt1 the second line point
     ...
     * if one of the lines is vertical use a distance dWidth above or below the line.
     * pass bolVertical1 = 1, or bolVertical2 = 1 if either line segment is vertical, else pass 0.
     * return the unique intersection in X,Y pointers.
     * p2 is the point that connects the 2 line segments to which the intersecting lines are related,
     * i.e. the intersecting lines are a distance dWidth pixels above or below p2.
     * uses dWidth and lOrient for cases in which at least one of the lines is vertical.
     * for normal lines this function assumes the caller has passed the m, b for the
     * appropriate upper or lower lines to get the desired intgercept.
     * this function is used for calculating the upper and lower channel lines for channel types.
     * For lOrient: see comments in Channels.ConnectTrueDouble2
     *
     ...
     * @param p2 point that connects the 2 line segments to which the intersecting lines are related
     ...
     * @param lOrient the orientation of the intersecting lines relative to the segments connecting p2
    ...
    /**
     * Original CELineArray design was based on an origin of 0,0 in the upper left corner.
     * This function translates points for the geo option due to calculation problems with 
     * negative pixel values.
     * @param pts OUT - points to translate
     * @param direction 0 to shift the points, 1 to shift them back by the smae amounts
     */
...
//    }
     ...
     * Computes an arc.
     *
     * @param pResultlinePoints OUT - contains center and start point and holds the result arc points
     * @param vblCounter the  number of client points
     * @param dRadius the arc radius in pixels
     * @param linetype the linetype determines start andgle and end angle for the arc
     *
     */
    ...
    /**
     * Computes the points for a circle.
     * Assumes CirclePoints has been allocated with size numpts.
     *
    ...
    /**
     * Gets the squall curves for a line segment
     ...
     * @param sign OUT - an object with a member to hold the starting curve sign for the segment.
     * @param amplitude the sin curve amplitutde
     * @param quantity the number of points for each sin curve
     * @param length the desired length of the curve along the segment for each sin curve
     *
			...
			int length)
    {
        ...
        try
        {
            ...
            if(numCurves==0)
            {
                pSquallPts[counter]=new POINT2(StartPt);counter++;
...
//    }
    /**
     * Analyzes if line from pt0 to pt 1 intersects a side and returns the intersection or null
     * assumes pt0 to pt1 is not vertical. the caller will replace pt0 with the intersection
     * point if it is not null
     * @param pt0
     * @param pt1
     ...
     *        ------
     *        side 3
     * bounds one segment for autoshapes that need it: bydif, fordif, fix, mnfldfix
     * if null is returned the client should conect the original line points (i.e. no jaggies)
     * @param pt0
     * @param pt1
     * @param ul
     * @param lr
...
//                bol1Inside=1;
    ...
    /**
     * Currently used on a limited basis and will likely be replaced
     * by BoundOneSegment if we do bound points.
     * @param pLinePoints - OUT - points to bound
     * @param vblCounter bound the first vblCounter points in the array.
     * @param segments - OUT - array of 0 or 1 for caller to use to determine whether
     * segment is to be drawn.
     *
     * @return the number of bounded points.
     */
...
//    }
    /**
     * Computes Ditch spikes for the ATDITCH line types. This function uses linestyles
     * provided by the caller to skip segments.
     *
     * @param pLinePoints OUT - the client points also used for the return points
     ...
     * the same distance from (and on the same side of) the the line as ptRelative.
     *
     * @param pt0 the first point
     * @param pt1 the second point
    ...
    /**
     * shift the control point to match the shift that occurs in Channels.GetAXADDouble
     * for CATKBYFIRE. This is because the rotary feature arrow tip must align with the anchor point
     * @param linetype 
     * @param pLinePoints the anchor points including the control point 
     * @param minDist the minimum required distance from the front of the rotary arrow
     */
    ...
    /**
     * Returns the point perpendicular to the line (pt0 to pt1) at atPoint
     * the same distance from (and on the same side of) the the line as ptRelative.
     *
     * @param pt0 the first point
     * @param pt1 the second point
     * @param atPoint the point on the line at which to compute the extended point
     * @param ptRelative the point to use for computing the return point
     *
     ...
     * would contain ptRelative. pt2 corresponds to pt0 and pt3 corresponds to pt1.
     *
    ...
    /**
     * Computes the distance in pixels from upper left to lower right of the minimum
     * bounding rectangle for the first numpts of pLinePoints
     *
